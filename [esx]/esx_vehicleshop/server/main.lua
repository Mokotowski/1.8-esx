...
RegisterServerEvent('esx_vehicleshop:soldcar')
AddEventHandler('esx_vehicleshop:soldcar', function (playerId, vehicleProps)
...
ESX.RegisterServerCallback('esx_vehicleshop:retrieveJobVehicles', function (source, cb, type)
	...
	MySQL.Async.fetchAll('SELECT * FROM police_vehicles WHERE owner = @owner AND type = @type AND job = @job', {
		['@owner'] = xPlayer.identifier,
		['@type'] = type,
...
AddEventHandler('esx_vehicleshop:setJobVehicleState', function(plate, state)
	...
	MySQL.Async.execute('UPDATE police_vehicles SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		['@stored'] = state,
		['@plate'] = plate,
		['@job'] = xPlayer.job.name
	}, function(rowsChanged)
