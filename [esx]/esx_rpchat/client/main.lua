...
AddEventHandler('esx_rpchat:sendProximityMessageTask', function(id, message, color)
	...
	elseif GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId(source)), GetEntityCoords(PlayerPedId(target)), true) < 50 then
		TriggerEvent('chat:addMessage', { args = { message }, color = color })
	end
end)
...
AddEventHandler('pokatwt', function(name1,name2,id, message)
	TriggerEvent('chat:addMessage1', "@" ..name1..' '..name2.." ", {24, 133, 217}, message, "fab fa-twitter")
end)
...
AddEventHandler('sendProximityMessageMe', function(id, name, message)
	...
	local ped = Citizen.InvokeNative(0x43A66C31C68491C0, pid)
    if IsEntityVisible(ped) then	
			TriggerEvent('chat:addMessage1', "^*"..name, {0, 255, 179}, " ".." " .. message, "fas fa-comment-dots", {255, 255, 255})
	end
end)
...
AddEventHandler('sendProximityMessageMed', function(id, name, message)
	...
	local ped = Citizen.InvokeNative(0x43A66C31C68491C0, pid)
    if IsEntityVisible(ped) then	
			TriggerEvent('chat:addMessage1', "^*"..name, {237, 63, 47}, " ".." " .. message, "fas fa-comment-dots", {255, 255, 255})
	end
end)
...
AddEventHandler('sendProximityMessageCzy', function(id, name, msg, czy)
	local pid = GetPlayerFromServerId(id)
	...
	if IsEntityVisible(ped) then
		if czy == 1 then
			TriggerEvent('chat:addMessage1',"^*🎲 [" .. name .. "] Udane", {19, 138, 51}, msg)
		elseif czy == 2 then
			TriggerEvent('chat:addMessage1',"^*🎲 [" .. name .. "] Nieudane", {173, 18, 3}, msg)
		end
	end	
end)
...
AddEventHandler('sendMessageToPlayer', function(id, name, message, gID)
  ...
  if pID == id then
	TriggerEvent('chat:addMessage1',"💬 ["..pID.."] "..name, {79, 114, 255}, message)
  end
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlPressed(1, 48) then
            ...
            for _, player in ipairs(GetActivePlayers()) do
                if  NetworkIsPlayerActive( player  and PlayerPedId( player ) ~= PlayerPedId()) then
                    ped = PlayerPedId( player )
                    blip = GetBlipFromEntity( ped )

                    x1, y1, z1 = table.unpack( GetEntityCoords( PlayerPedId(), true ) )
                    x2, y2, z2 = table.unpack( GetEntityCoords( PlayerPedId( player ), true ) )
