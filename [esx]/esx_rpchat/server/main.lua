...
AddEventHandler('chatMessage', function(source, name, message)
	if string.sub(message, 1, string.len("/")) ~= "/" then
		...
		TriggerEvent('top_discord:send', source, 'https://discord.com/api/f//8jR3l1ksXG0CaD9g9OHmDWs3IsjYAePWuLeUrbHoFZ0z26XCs3G0E7e8PghXhx-zqRIq', 'Napisał na looc: '..message)
	end
	CancelEvent()
end)
...
RegisterCommand('try', function(source, args, rawCommand)
	...
	TriggerEvent('top_discord:send', source, 'https://discord.com/d/webhooks/848227484247523368/WO8l-opnMsEL7NHjCrfASRXoTkNFuLzPwYe4i3zxMmT37b-q5HOGDRxVurY6AymUmKbg', 'Napisał na try odpowiedz : '.. czy)
end)
...
RegisterCommand('w', function(source, args, rawCommand)
    ...
    if pID ~= nil then
        if GetPlayerPing(pID) == 0 then
			TriggerClientEvent('FeedM:showNotification', xPlayer.source, 'Podane ID jest offline!')
        else
            if args[2] ~= nil then
                local text = table.concat(args, " ")
                local msg = string.gsub(text, args[1] .. ' ', "")
                TriggerClientEvent("sendMessageToPlayer", -1, source, name, msg)
                TriggerClientEvent("sendMessageToPlayer", -1, pID, name, msg)
            else
				TriggerClientEvent('FeedM:showNotification', xPlayer.source, 'Podej treść którą chcesz wysłać do danej osoby!')

            end
        end
    else
		TriggerClientEvent('FeedM:showNotification', xPlayer.source, 'Podaj ID osoby do której chcesz napisać!')
...
RegisterCommand('me', function(source, args, rawCommand)
	...
	local color = {r = 0, g = 255, b = 179}
	TriggerClientEvent('esx_rpchat:triggerDisplay', -1, text, source, color)
	local xPlayer = ESX.GetPlayerFromId(source)
	local steamid = xPlayer.identifier
	local name = GetPlayerName(source)

	TriggerEvent('top_discord:send', source, 'https://discord.com/d/webhooks/848227689881665596/rBxJBSw-aLe97gJB6FpwVlk4laSd4cbX_RUCN2v29tWSFdQihW1gjlHonquohtHL5fmA', 'Napisał na me : '..text)
end, false)
...
RegisterCommand('med', function(source, args, rawCommand)
	...
	local xPlayer = ESX.GetPlayerFromId(source)
	local czas = os.date("%Y/%m/%d %X")
    TriggerClientEvent("sendProximityMessageMed", -1, source, source, table.concat(args, " "))

	local text = ''
    for i = 1,#args do
        text = text .. ' ' .. args[i]
    end
	local color = {r = 237, g = 63, b = 47}
	TriggerClientEvent('esx_rpchat:triggerDisplay', -1, text, source, color)
	local xPlayer = ESX.GetPlayerFromId(source)
	local steamid = xPlayer.identifier
	local name = GetPlayerName(source)
...
RegisterCommand('do', function(source, args, rawCommand)
	...
	local color = {r = 255, g = 0, b = 239}
	TriggerClientEvent('esx_rpchat:triggerDisplay', -1, text, source, color)
	local xPlayer = ESX.GetPlayerFromId(source)
	local steamid = xPlayer.identifier
	local name = GetPlayerName(source)
	TriggerEvent('top_discord:send', source, 'https://discord.com/d/webhooks/848238262546858084/qrwsxwGVAjW_5jBHgPhDdSIln0VNaRcBxiQsEdy9Q6NgJbzh49ftCbyFaE6PBDB-IKT8', 'Napisał na do : '..text)
end, false)
...
AddEventHandler('sendTwtServer', function(message)
	...
	if mamhajsikbejbe > 100 then
			local name = getIdentity(_source)
			local name1 = name.firstname
			local name2 = name.lastname
		TriggerClientEvent("pokatwt", -1, name1,name2, message, message)

		xPlayer.removeAccountMoney('bank', 100)
		TriggerClientEvent('FeedM:showNotification', xPlayer.source, '~y~Pobrano opłatę~r~ 100$')
	else
		TriggerClientEvent('FeedM:showNotification', xPlayer.source, '~y~Nie masz pieniędzy w banku')
	end
end)
...
RegisterCommand('police', function(source, args, rawCommand)
	...
	if args[1] ~= nil then
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('chat:addMessage1', -1, "👮 @" ..name.firstname..' '..name.lastname.."", {30, 144, 255}, msg)
				TriggerEvent('top_discord:send', source, 'https://discord.com/api/webhooks//-B1-WDFtdwkb85tsEUMdjt-nAqUJ8JCk68vLDOBxM9h8qJ2OQXpzs-hmjLdqDM', 'Napisał na SASP : ' ..msg)
		end
	end
end)
...
RegisterCommand('o', function(source, args, rawCommand)
	if args[1] ~= nil then
		...
		if xPlayer.group == 'superadmin' or xPlayer.group == 'admin' or xPlayer.group == 'best' or xPlayer.group == 'moderator' or xPlayer.group == 'support' or xPlayer.group == 'trialsupport' then
			...
			msg = Emojit(table.concat(args, " "))
			TriggerClientEvent('chat:addMessage1', -1,"🔔 ^*" .. playerName .. " ", {255, 0, 0}, msg)

			TriggerEvent('top_discord:send', source, 'https://discord.com/api/webhooks//', 'Napisał na ogłoszeniach : ' ..msg)
		end
	end
end)
...
RegisterCommand('news', function(source, args, rawCommand)
	...
	if money2 > 10000 then
		if quantity > 0 then
 		   	fal = "📰 " .. imie
				TriggerClientEvent('chat:addMessage1', -1,"^* " .. fal .. " ", {255, 153, 0}, msg)
				xPlayer.removeAccountMoney('bank', 10000)
			TriggerClientEvent('FeedM:showNotification', xPlayer.source, '~y~Pobrano opłatę ~r~$10000')
			TriggerEvent('top_discord:send', source, 'https://discord.com/api/webhooks//-fT5aIIiluddwhndcAcehpPJzN_iofcIzq3WpOWwz4F', 'Napisał na newsie : ' ..msg)
		else
			TriggerClientEvent('FeedM:showNotification', xPlayer.source, 'Nie masz wystarczająco pieniędzy w banku')
		end
	else
		TriggerClientEvent('FeedM:showNotification', xPlayer.source, 'Nie możesz pisać ogłoszenia bez telefonu')
	end
end, false)
...
AddEventHandler('senddarkServer', function(message)
	...
	local name = getIdentity(_source)
	local mamhajsikbejbe = xPlayer.getAccount('bank').money
	if mamhajsikbejbe > 200 then
		TriggerClientEvent("pokaDw", -1, -1, message)
	    
		xPlayer.removeAccountMoney('bank', 200)
		TriggerClientEvent('FeedM:showNotification', xPlayer.source, '~y~Pobrano opłatę~r~ 200$')
	else
		TriggerClientEvent('FeedM:showNotification', xPlayer.source, '~y~Nie masz siana w banku')
	end
end)
