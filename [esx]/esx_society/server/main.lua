...
AddEventHandler('esx_society:withdrawMoney', function(society, amount)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
		if amount > 0 and account.money >= amount then
			...
			sendToDiscord('esx_society','[BOSS MENU] ' .. GetPlayerName(source) .. ' - Wypłacił hajs, ILOŚĆ: ' .. amount, 16711680)
		else
...
AddEventHandler('esx_society:depositMoney', function(society, amount)
	...
	if amount > 0 and xPlayer.getMoney() >= amount then
		...
		sendToDiscord('esx_society','[BOSS MENU] ' .. GetPlayerName(source) .. ' - Wpłacił hajs, ILOŚĆ: ' .. amount, 16711680)
	else
		TriggerClientEvent('esx:showNotification', xPlayer.source, _U('invalid_amount'))
	end
end)

RegisterServerEvent('esx_society:ubezpieczenieS')
AddEventHandler('esx_society:ubezpieczenieS', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	local society = GetSociety('ambulance')
	local kwota = 10
	TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
		xPlayer.removeAccountMoney('bank', kwota)
		account.addMoney(kwota)
	end)
	TriggerClientEvent("pNotify:SendNotification", xPlayer.source, {text = 'Zapłacono $'..kwota..' jako Ubezpieczenie Zdrowotne'})
end)
...
AddEventHandler('esx_society:washMoney', function(society, amount)
	...
	if amount and amount > 0 and account.money >= amount then
			...
			TriggerClientEvent('esx:showNotification', xPlayer.source, (' ~g~$' .. ESX.Math.GroupDigits(amount) .. '~y~ pranie pieniędzy~s~ (' .. howlong .. minorsec .. ').'))
			Citizen.Wait(1000 * xtime * howlong)
			xPlayer.addMoney(amount)
			TriggerClientEvent('esx:showNotification', xPlayer.source, ('Gotowe!! ~g~$' .. ESX.Math.GroupDigits(amount) .. '~s~ dostałeś ~y~gotówkę~s~'))
...
function sendToDiscord (name,message,color)
  local DiscordWebHook = 'https://discord.com/api/webhooks/832350090206249030/IoQ6y0u6lPRJBpaDj-6UJQUZYMbzlM-Cgkk6yb3dvXV9BeurYxGH9ivNZxvTp7TgQWmo'
