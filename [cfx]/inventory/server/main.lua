        ...
        if item.type == 'item_account' then
            if item.name == 'cash' then
                if target.getMoney() >= count then
                    ...
                    TriggerClientEvent('inventory:notify', src, 'success', ('%s <b>%s</b> Został przeniesiony <b>%s</b>!'):format(count, item.label, 'Do twojego ekwipunku'))
        ...
        elseif item.type == 'item_weapon' then
            if target.hasWeapon(item.name) then
                if not xPlayer.hasWeapon(item.name) then
                    ...
                    xPlayer.addWeapon(item.name, item.count)
                    TriggerClientEvent('inventory:notify', src, 'success', ('<b>%s</b> Został przeniesiony <b>%s</b>!'):format(item.label, 'Do twojego ekwipunku'))
        ...
        if item.type == 'item_account' then
            if item.name == 'cash' then
                if xPlayer.getMoney() >= count then
                    ...
                    TriggerClientEvent('inventory:notify', src, 'success', ('%s <b>%s</b> Przeniesiony do <b>%s</b>!'):format(count, item.label, otherInv.title))
        ...
        elseif item.type == 'item_weapon' then
            if xPlayer.hasWeapon(item.name) then
                if not target.hasWeapon(item.name) then
                    ...
                    TriggerClientEvent('inventory:notify', src, 'success', ('<b>%s</b> Przeniesiony do <b>%s</b>!'):format(item.label, otherInv.title)) 
...
RegisterNetEvent('inventory:removeItem', function(item, count, coords)
    ...
    if item.type == 'item_account' then
        if item.name == 'cash' then
            if xPlayer.getMoney() >= count then
                ...
                TriggerClientEvent('inventory:notify', src, 'success', ('%s <b>%s</b> <b>%s</b>!'):format(count, item.label, 'Wyrzucony'))
    ...
    elseif item.type == 'item_weapon' then
        if xPlayer.hasWeapon(item.name) then
            ...
            TriggerClientEvent('inventory:notify', src, 'success', ('<b>%s</b> <b>%s</b>!'):format(item.label, 'Wyrzucony'))
...
ESX.RegisterServerCallback('inventory:danedlaequ', function(source, cb)

    local xPlayer = ESX.GetPlayerFromId(source)

    local result = MySQL.Sync.fetchAll('SELECT wyborequ FROM users WHERE identifier = @identifier', {
        ['@identifier'] = xPlayer.identifier
    })

    local wyborequ = result[1].wyborequ
    cb(wyborequ)
    --print(wyborequ)
...
RegisterServerEvent("inventory:savess")
AddEventHandler("inventory:savess", function(a)
    local b = source;
    local c = ESX.GetPlayerFromId(b)
    MySQL.Async.insert("UPDATE users SET wyborequ = @wyborequ", {["@wyborequ"] = a})
