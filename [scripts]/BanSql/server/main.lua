...
TriggerEvent('es:addGroupCommand', 'sqlban', Config.Permission, function (source, args, user)
	...
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM ', 'Niewystarczające uprawnienia.' } })
	...
	if target and target > 0 then
		...
		if ping and ping > 0 then
			if duree and duree < 365 then
				...
				print("BanSql Error : Niepoprawny czas Auto-Cheat-Bana.")
			end	
		else
			print("BanSql Error : Cel Auto-Cheat-Ban nie jest online.")
		end
	else
		print("BanSql Error : Auto-Cheat-Ban otrzymał nieprawidłowy identyfikator.")
	end
end)
