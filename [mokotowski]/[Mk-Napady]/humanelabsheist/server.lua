...
ESX.RegisterServerCallback('humanelabsheist:server:checkTime', function(source, cb)
    local src = source
    local player = ESX.GetPlayerFromId(src) -- sta/sz/ek#/96/09
...
AddEventHandler('humanelabsheist:server:heistRewards', function()
    local src = source -- sta/sz/ek#/96/09
    local player = ESX.GetPlayerFromId(src)
    ...
    if player then
        if start then
            ...
            if Config['HumaneLabs']['rewards']['bank'] > 0 then
                player.addAccountMoney('bank', Config['HumaneLabs']['rewards']['bank'])
                discordLog(player.getName() ..  ' - ' .. player.getIdentifier(), ' gain $' .. Config['HumaneLabs']['rewards']['bank'] .. 'black money')
            end

            if Config['HumaneLabs']['rewards']['items'] ~= nil then
                for k, v in pairs(Config['HumaneLabs']['rewards']['items']) do
                    local rewardCount = v['count']() -- sta/sz/ek#/96/09
