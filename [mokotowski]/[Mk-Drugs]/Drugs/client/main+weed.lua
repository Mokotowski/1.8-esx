    ...
    function()
        while true do
            ...
            for i, j in pairs(Config.Teleports) do
                ...
                for k = 1, #j.TeleportForAmfetaminIN, 1 do
                    local l = #(g - j.TeleportForAmfetaminIN[k])
                    if l < 15 then
                        ESX.Game.Utils.DrawText3D(j.TeleportForAmfetaminIN[k], Config.Translate[2], 0.6)
                        h = false
                        if l < 1 then
                            if IsControlJustReleased(0, 38) then
                                DoScreenFadeOut(100)
                                Citizen.Wait(750)
                                ESX.Game.Teleport(f, j.TeleportForAmfetaminOUT[k])
                                DoScreenFadeIn(100)
                            end
                        end
                    end
                end
                for k = 1, #j.TeleportForAmfetaminOUT, 1 do
                    local l = #(g - j.TeleportForAmfetaminOUT[k])
                    if l < 15 then
                        ESX.Game.Utils.DrawText3D(j.TeleportForAmfetaminOUT[k], Config.Translate[1], 0.6)
                        h = false
                        if l < 1 then
                            if IsControlJustReleased(0, 38) then
                                DoScreenFadeOut(100)
                                Citizen.Wait(750)
                                ESX.Game.Teleport(f, j.TeleportForAmfetaminIN[k])
                                DoScreenFadeIn(100)
                            end
                        end
                    end
                            ...
                            if IsControlJustReleased(0, 38) and not d then
                                if rawCocaine >= 1 then
                                    ESX.TriggerServerCallback(
                                        "esegovic:sprawdźmieszanyprodukt",
                                        function(w)
                                            if w == true then
                                                d = true
                                                MixCocaine()
                        ...
                        if l < 1.5 then
                            if IsControlJustReleased(0, 38) then
                                if meth3 >= 10 then
                                    ...
                                    if z > 27 then
                                        TriggerServerEvent("esegovic:powiadomieniePolicji", y)
                                    end
                                else
...
Esegovicmenu = function()
    ...
    ESX.UI.Menu.Open(
        ...
        "drying",
        {
            ...
            align = "bottom-right",
            elements = {
                {label = "| słaba |", value = "indika"},
                {label = "| średnia |", value = "sativa"},
                {label = "| mocna |", value = "supak"}
            }
        },
        function(D, E)
            ...
            if D.current.value == "sativa" then
                if z > 27 then
                    TriggerServerEvent("esegovic:powiadomieniePolicji")
                end
                ESX.TriggerServerCallback(
...
PackWeed = function()
    ...
    ESX.UI.Menu.Open(
        ...
        "packing",
        {
            ...
            align = "bottom-right",
            elements = {
                {label = "| Paczka słabej |", value = "pack_indica"},
                {label = "| Paczka średniej |", value = "pack_sativa"},
                {label = "| Paczka mocnej |", value = "pack_purple"}
            }
        },
        function(D, E)
...
PackIndica = function()
    exports[Config.FolderNameMythicProgbar]:Progress(
        ...
        function(A)
            if not A then
                ClearPedTasks(PlayerPedId())
                TriggerServerEvent("esegovic:paczkaIndica")
            else
                ClearPedTasks(PlayerPedId())
            end
        end
