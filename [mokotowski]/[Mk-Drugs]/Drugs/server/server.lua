        ...
        if neaW() then
            if Config.UseNewESX then
                if j.canCarryItem("weed_indica", 1) then
                    ...
                    TriggerClientEvent('esx:showNotification', source, "~g~Wspaniale~s~! Własnie przyciąłeś roślinę.")

                else
                    TriggerClientEvent('esx:showNotification', source, "Nie masz więcej miejsca.")
                end
            else
                j.addInventoryItem("weed_indica", 1)
                TriggerClientEvent('esx:showNotification', source, "~g~Wspaniale~s~! Własnie przyciąłeś roślinę.")
        ...
        if neaW() then
            if Config.UseNewESX then
                if j.canCarryItem("indica_weed", m) then
                    ...
                    TriggerClientEvent('esx:showNotification', source, "YOczyściłeś i zapakowałeś zioło, masz swój produkt końcowy.:~y~ " .. m .. "g")
                ...
                j.addInventoryItem("indica_weed", m)
                TriggerClientEvent('esx:showNotification', source, "Oczyściłeś i zapakowałeś zioło, masz swój produkt końcowy.:~y~ " .. m .. "g")
            end
        else
            print("Cheater Triggering Event!")
        end
...
AddEventHandler(
    ...
    function()
        ...
        if neaC() then
            if Config.UseNewESX then
                if j.canCarryItem("cocaine", 1) then
                    j.addInventoryItem("cocaine", 1)
                    TriggerClientEvent('esx:showNotification', source, "Udało Ci się zapakować ~g~10g~s~ ~y~Kokainy.~s~.")

                else
                    TriggerClientEvent('esx:showNotification', source, "Nie masz więcej miejsca.")
                end
            else
                j.addInventoryItem("cocaine", 10)
                TriggerClientEvent('esx:showNotification', source, "Udało Ci się zapakować ~g~10g~s~ ~y~Kokainy.~s~.")
            end
        else
            print("Cheater Triggering Event!")
...
AddEventHandler(
    ...
    function()
        local j = ESX.GetPlayerFromId(source)
        if neaA() then
            j.addInventoryItem("amfetamin", 5)
            TriggerClientEvent('esx:showNotification', source, "Niesamowite! Właśnie wyprodukowałeś 5 g amfetaminy..")
