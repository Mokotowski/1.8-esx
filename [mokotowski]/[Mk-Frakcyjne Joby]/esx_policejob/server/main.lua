...
AddEventHandler('esx_policejob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	elseif itemType == 'item_weapon' then
		...
		if targetXPlayer.hasWeapon(itemName) then
			targetXPlayer.removeItem(itemName, amount)
...
ESX.RegisterServerCallback('esx_policejob:getVehicleInfos', function(source, cb, plate)
	MySQL.Async.fetchAll('SELECT owner FROM police_vehicles WHERE plate = @plate', {
		['@plate'] = plate
	}, function(result)
	...
	if not selectedWeapon then
		...
		if type == 1 then
			if xPlayer.getMoney() >= selectedWeapon.price then
				...
				xPlayer.addInventoryItem(weaponName, 100)
...
ESX.RegisterServerCallback('esx_policejob:buyJobVehicle', function(source, cb, vehicleProps, type)
	...
	if price == 0 then
		...
		if xPlayer.getMoney() >= price then
			...
			MySQL.Async.execute('INSERT INTO police_vehicles (owner, vehicle, plate, type, job, `stored`) VALUES (@owner, @vehicle, @plate, @type, @job, @stored)', {
...
ESX.RegisterServerCallback('esx_policejob:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	...
	for k,v in ipairs(nearbyVehicles) do
		local result = MySQL.Sync.fetchAll('SELECT plate FROM police_vehicles WHERE owner = @owner AND plate = @plate AND job = @job', {
			['@owner'] = xPlayer.identifier,
			['@plate'] = v.plate,
			['@job'] = xPlayer.job.name
		})
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE police_vehicles SET `stored` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			['@owner'] = xPlayer.identifier,
			['@plate'] = foundPlate,
			['@job'] = xPlayer.job.name
		}, function (rowsChanged)
