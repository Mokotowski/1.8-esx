...
RegisterCommand("przywolajpojazd", function()
...
function OpenSpawnerMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
		title = 'Dostarczanie Pojazdu',
		align = GetConvar('esx_MenuAlign', 'top-left'),
		elements = {
			{label = ('Dostępne Pojazdy'), value = 'veh_menu'}
	}}, function(data, menu)
		...
		if action == 'veh_menu' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'veh_menu', {
				...
				ESX.UI.Menu.CloseAll()
				if enroute then
					ESX.ShowNotification("Musisz poczekać przed ponownym wezwaniem pojazdu")
					return
				end
				...
				for i = 1, #gameVehicles do
					...
					if DoesEntityExist(vehicle) then
						...
						ESX.ShowNotification("Pojazd jest poza garażem.")
							return
						end
					end
				end
				ESX.ShowNotification("Zapłaciłeś 100 dolarów za przywóz pojazdu!")
